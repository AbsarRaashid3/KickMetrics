...
const deletePlayer = (index) => {
  ...
  return (
    <Container className="dashboardC">
      <Row>
  ...
  <p><strong>Vision:</strong> {selectedPlayer.vision}</p>
      ...
      <Row>
        ...
        <Col md={6} className="sticky-notes">
          <h4>Sticky Notes</h4>
          {notes.map((note, index) => (
            <div key={index} className="note">
              <Form.Control value={note} onChange={(e) => updateNote(index, e.target.value)} />
              <Button variant="danger" onClick={() => deleteNote(index)}>X</Button>
            </div>
          ))}
      ...
      <Row>
        <Col md={6} className="next-games">
          <h4>Next Game</h4>
          {games.map((game, index) => (
            <div key={index}>
              <Form.Control value={game.opponent} onChange={(e) => updateGame(index, "opponent", e.target.value)} />
              <Form.Control type="date" value={game.date} onChange={(e) => updateGame(index, "date", e.target.value)} />
              <Button variant="danger" onClick={() => deleteGame(index)}>X</Button>
            </div>
          ))}
  ...
  <Col md={6} className="my-team">
    ...
    <Form.Select 
      ...
      disabled={myTeam.length >= 11} // Disable when 11 players are added
    >
      <option value="">Select a Player</option>
      {players
        .filter((p) => !myTeam.some((teamPlayer) => teamPlayer._id === p._id)) // Prevent duplicates
        .map((p) => (
    ...
    <Button 
      onClick={addPlayer} 
      disabled={!selectedPlayerId || myTeam.length >= 11} // Disable if no selection or limit reached
    >
      Add Player
    </Button>
    ...
    {myTeam.map((player, index) => (
      <div key={index} style={{ marginBottom: "10px", padding: "10px", border: "1px solid #ccc", borderRadius: "5px" }}>
        ...
        <Button variant="danger" onClick={() => deletePlayer(index)}>Remove</Button>
      </div>
    ))}
  </Col>
</Row>
